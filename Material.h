...
class Material {
public:
    inline Material () : diffuse (0.8f), specular (0.2f), color (0.5f, 0.5f, 0.5f),isMirror(false)  {}
    ...
    inline bool getMirror ()const { return isMirror; }

    inline void setDiffuse (float d) { diffuse = d; }
    inline void setSpecular (float s) { specular = s; }
    inline void setColor (const Vec3Df & c) { color = c; }
    inline void setMirror (bool _isMirror) { isMirror = _isMirror; }
private:
    float diffuse;
    float specular;
    bool isMirror;
