...
void drawBbox(const BoundingBox & bb) {
	Vec3Df bbmin = bb.getMin();
	Vec3Df bbmax = bb.getMax();
	glBegin(GL_LINES);
...
void GLViewer::draw () {
    ...
    for (unsigned int i = 0; i < scene->getObjects ().size (); i++) {
		...
		o.getKDTree()->getRecursiveBboxes(2, bbs);
		for (unsigned int j = 0; j < bbs.size(); j++) drawBbox(bbs[j]);
